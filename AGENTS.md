...
- Current experiment interpretation must respect the project's custom joint semantics rather than assuming standard OpenPose18 names. In the user's custom mapping, joints `10` and `12` are the two wrist-like failure points, and custom wrist-chain direction supervision uses edges `(8, 10)` and `(12, 13)`.
...
- `--distal-loss-weight` adds target-domain supervision on OpenPose18 wrist/knee/ankle joints `(4, 7, 9, 10, 12, 13)`. `--distal-replay-factor` repeats few-shot target frames with high distal-joint pose variation so small, weak-scattering limb states are not under-sampled. `--source-replay-weight` adds supervised source-domain replay during finetune to keep the model from collapsing toward the target few-shot mean pose while distal target loss is emphasized.
- `--wrist-direction-loss-weight` adds target-domain direction supervision on the custom wrist chains `(8 -> 10)` and `(12 -> 13)`. It compares normalized limb directions rather than absolute wrist coordinates, testing whether target-domain wrist PCK is limited by wrong arm-chain direction under weak wrist scattering rather than by small coordinate offsets alone.
- `--wrist-refinement` adds an identity-initialized local residual head after the decoder. It reads decoder joint latents for custom hand/wrist context joints `(8, 10, 12, 13)` and only adjusts custom wrist-like endpoints `10` and `12`. This tests whether cross-domain PCK is limited by weak wrist-reflection endpoint offsets after the coarse full-body pose is already roughly correct, while preserving stable torso/head/leg predictions by default. Use `--trainable-groups wrist_refiner` first to isolate the new head, then compare against `wrist_refiner decoder` and `full`.
...
Run target-domain input calibration from a release2.0-style source checkpoint when testing whether env-specific antenna/subcarrier amplitude response limits cross-domain transfer:
...
Run distal-joint hard target replay with source replay when testing whether wrist/knee/ankle target-domain supervision coverage limits cross-domain PCK:

```powershell
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_distal_replay_source_reg --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups full --distal-loss-weight 0.5 --distal-replay-factor 3 --distal-replay-fraction 0.5 --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 50 --batch-size 64
```

Run custom wrist-chain direction supervision with source replay when joints 10 and 12 dominate target-domain PCK errors:

```powershell
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_wrist_direction_source_reg --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups full --wrist-direction-loss-weight 0.25 --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 50 --batch-size 64
```

Run custom wrist local refinement when joints 10 and 12 remain the dominant target-domain failure after coarse full-body finetuning:

```powershell
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_wrist_refiner_only --finetune-from outputs\source_baseline\best_val_mpjpe.pth --wrist-refinement --trainable-groups wrist_refiner --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 50 --batch-size 64
```
...
# Optional Phase 3 variant: distal hard target replay with source replay regularization
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\finetune_distal_replay_source_reg --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups full --distal-loss-weight 0.5 --distal-replay-factor 3 --distal-replay-fraction 0.5 --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30

# Optional Phase 3 variant: custom wrist-chain direction loss with source replay regularization
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\finetune_wrist_direction_source_reg --finetune-from outputs\source_baseline\best_val_mpjpe.pth --trainable-groups full --wrist-direction-loss-weight 0.25 --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30

# Optional Phase 3 variant: custom wrist local refinement only
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\finetune_wrist_refiner_only --finetune-from outputs\source_baseline\best_val_mpjpe.pth --wrist-refinement --trainable-groups wrist_refiner --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30
...
# Wrist refiner only: test whether a local residual head can correct weak custom wrist endpoints without disturbing coarse pose
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_wrist_refiner_only --finetune-from outputs\source_baseline\best_val_mpjpe.pth --wrist-refinement --trainable-groups wrist_refiner --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30

# Wrist refiner + decoder: adapt the local endpoint correction and decoder pose readout together
python train.py --mode finetune --dataset-root data\mmfi_pose --source-envs env1 --target-envs env2 --output-dir outputs\ft_wrist_refiner_decoder --finetune-from outputs\source_baseline\best_val_mpjpe.pth --wrist-refinement --trainable-groups wrist_refiner decoder --source-replay-weight 0.25 --few-shot-subjects 4 --few-shot-frames 5 --epochs 30

# Input calibration + encoder: calibrate target CSI and let spatial/axial features adapt
python train.py --mode finetune --dataset-root data\mmfi_pose --target-envs env2 --output-dir outputs\ft_input_calibration_encoder --finetune-from outputs\source_baseline\best_val_mpjpe.pth --input-calibration antenna_subcarrier_affine --trainable-groups input_calibration encoder --few-shot-subjects 4 --few-shot-frames 5 --epochs 30
...
Supported `--trainable-groups` values are `encoder`, `decoder`, `full`, `input_calibration`, `wrist_refiner`, `spatial_encoder`, `axial_encoder`, `axial_attention`, `axial_projection`, `decoder_queries`, `decoder_attention`, `decoder_head`, `decoder_norms`, and `norms`. `--freeze-tier` remains only as a deprecated compatibility option for older cumulative-unfreeze runs; prefer `--trainable-groups` for rigorous ablations.
...
Training and evaluation outputs are written under `outputs/` by default. Checkpoints include `best_val_mpjpe.pth`, `best_val_pck_0_2.pth`, and `last.pth`. Completed epoch metrics are appended to the compatibility `train_log.csv` and the dedicated `loss.csv`, `mpjpe.csv`, `bone_error.csv`, and `pck_0_1.csv` through `pck_0_5.csv` files. Evaluation visualizations are saved only as `.png` files grouped by action/environment samples; evaluation does not generate PDF figures.
